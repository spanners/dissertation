...
var SM=window.SM||{};SM.Object={create:function(obj){function F(){}
...
return true;}};if(window.Object.create){SM.Object.create=window.Object.create;};SM.Array={indexOf:function(arr,item){var i,len=arr.length;for(i=0;i<len;i++){if(arr[i]===item){return i;}}
...
return 0;}else{return b-a;}},unique:function(arr){var o={},i,val,key,l=arr.length,r=[];for(i=0;i<l;i++){val=arr[i];o[val]=val;}
for(key in o){r.push(o[key]);}
return r;},addUnique:function(arr,item){var i=this.indexOf(arr,item);if(i===-1){arr.push(item);}},removeItem:function(arr,item){var i=this.indexOf(arr,item);if(i!==-1){arr.splice(i,1);}}};SM.String={r_http_url:/^(https?:\/\/)/,init:function(){this.rtrim=/\S/.test("\xA0")?(/^[\s\xA0]+|[\s\xA0]+$/g):/^\s+|\s+$/g;},truncate:function(str,maxLength,addEllispses){if(!str){return"";}
if(!maxLength){return str;}
if(addEllispses===undefined){addEllispses=true;}
if(str.length>maxLength){if(addEllispses){return str.substring(0,maxLength-3)+"...";}
...
return str.substr(i+1);},beforeFirst:function(str,s){var i;if(!str){return"";}
i=str.indexOf(s);if(i===-1){return str;}
return str.substr(0,i);},isHttpUrl:function(str){return this.r_http_url.test(str);},trim:String.prototype.trim?function(str){return str===null?"":String.prototype.trim.call(str);}: function(str){return str===null?"":str.toString().replace(this.rtrim,"");}};SM.String.init();SM.Math={roundTo:function(num,places){return this._roundTo(num,places,0);},roundPercentageTo:function(num,places){return this._roundTo(num,places,2);},isInt:function(num){if(!isFinite(num)||(typeof num).toLowerCase()!=="number"){return false;}
...
top=Math.pow(10,places+multiplier)*(num);bottom=Math.pow(10,places);return(Math.round(top)/bottom);}};SM.DOM={SPACE:' ', getClass:function(el){return el.className?el.className:"";}, hasClass:function(el,className){var currentClassNames;var classNames;var i=0;var classNamesLen;if(!el){return false;}
...
return returnEls;},getCoordinates:function(el,containerEl){var t=0;var l=0;do{l+=el.offsetLeft;t+=el.offsetTop;el=el.offsetParent;if(containerEl&&el===containerEl){break;}}while(el);return{top:t,left:l};}};SM.Event={normalize:function(e){if(e.srcElement){e.target=e.srcElement;}
...
if(el.addEventListener){el.addEventListener(eventType,normalizedCallback,false);}else if(el.attachEvent){el.attachEvent(on+eventType,normalizedCallback);}else if(el[on+eventType]){el[on+eventType]=normalizedCallback;}},BLUR:"blur",CHANGE:"change",CLICK:"click",ENTER_KEYPRESS:"enterkeypress",FOCUS:"focus",FOCUSIN:"focusin",FOCUSOUT:"focusout",KEYDOWN:"keydown",KEYPRESS:"keypress",KEYUP:"keyup",MOUSEDOWN:"mousedown",MOUSEENTER:"mouseenter",MOUSELEAVE:"mouseleave",MOUSEMOVE:"mousemove",MOUSEUP:"mouseup",PASTE:"paste",RESIZE:"resize",SUBMIT:"submit"};(function(window,undefined){
...
switch(typeof value){case'string':return quote(value);case'number':return isFinite(value)?String(value):'null';case'boolean':case'null':
return String(value);
